...
def apply_classifier(x, model, img, im0):
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            pred_cls2 = torch.zeros((len(ims),96)).to(d.device)
            for m in model:
                # m.eval()
                m.to(d.device)
                # m.eval()
            ...
            # print(pred_cls2)
            # mask = (pred_cls2 == pred_cls2.max(dim=1, keepdim=True)[0])
            # print(mask*1)
    res = torch.zeros(96, dtype=torch.float32)
    for cls in pred_cls2:
        tes = onehot(96,cls)
        res += tes
    # print(res)
