...
def run(
    ...
    map_path = r'data/label_map.csv'
    title = pd.read_csv(map_path).name
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        predc = apply_classifier(pred, modelcs, im, im0s)

        indexname = str(s).split('\\')[-1].split('.')[0]
        predc = predc.numpy()
        dataframe.loc[indexname,:] = predc
        # print(dataframe)
    ...
    dataframe.to_csv(os.path.join(save_dir,'result.csv'))
...
def parse_opt():
    ...
    parser.add_argument('--iou-thres', type=float, default=0.55, help='NMS IoU threshold  CIou')
    parser.add_argument('--max-det', type=int, default=1000, help='maximum detections per image')
    parser.add_argument('--device', default='', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    parser.add_argument('--view-img', action='store_true', help='show results') #实时演示
    parser.add_argument('--save-txt', action='store_true', help='save results to *.txt') #保存坐标
    parser.add_argument('--save-conf', action='store_true', help='save confidences in --save-txt labels')
    parser.add_argument('--save-crop', action='store_true', help='save cropped prediction boxes')
    parser.add_argument('--nosave', action='store_true', help='do not save images/videos')
    parser.add_argument('--classes', nargs='+', type=int, help='filter by class: --classes 0, or --classes 0 2 3') #类别限定
